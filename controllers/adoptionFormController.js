...
const addAdoptionForm = async (req, res) => {
  ...
  try {
    ...
    return res
      ...
      .json({ messsage: "Pomyślnie zgłoszono wniosek o adopcje" });
  } catch (error) {
    return res
  ...
  try {
    ...
    if (!deleted) {
      return res
        ...
        .json({ message: "Wniosek nie istnieje" });
    }
...
const acceptAdoptionForm = async (req, res) => {
  const id = +req.params.id;
  try {
    const form = await AdoptionFormService.getById(id);
    if (!form) {
      return res
        .status(StatusCodes.NOT_FOUND)
        .json({ message: "Wniosek nie istnieje" });
    ...
    await AdoptionFormService.markPetAsAdopted(form.id_pet);
